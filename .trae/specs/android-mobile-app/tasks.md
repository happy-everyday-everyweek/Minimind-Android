...
- [ ] Task 1: 后端 API 增强 - 扩展 serve_openai_api.py
  ...
  - [ ] SubTask 1.3: 添加 CORS 中间件支持
  - [ ] SubTask 1.4: 添加可选 Bearer Token 鉴权中间件，新增 `--api_key` 启动参数
  - [ ] SubTask 1.5: 验证现有 `/v1/chat/completions` 端点功能不受影响

- [ ] Task 2: 后端部署脚本
  - [ ] SubTask 2.1: 创建 `scripts/deploy_linux.sh` 一键部署脚本（安装依赖、配置防火墙、启动服务）
  - [ ] SubTask 2.2: 创建 systemd service 文件模板，支持 start/stop/restart

- [ ] Task 3: Android 项目初始化
  - [ ] SubTask 3.1: 创建 Android 项目目录结构 `android/`，配置 Gradle、Kotlin、Jetpack Compose
  - [ ] SubTask 3.2: 配置项目依赖：Retrofit + OkHttp（网络）、Room（本地存储）、Navigation（导航）、Material3（UI）
...
- [ ] Task 7: Android 设置页面实现
  - [ ] SubTask 7.1: 实现服务器配置（地址、API Key、连接测试）
...
- [ ] Task 8: Android 导航与整体集成
  ...
  - [ ] SubTask 8.3: 端到端集成测试：Android 客户端连接后端 API 完成一次完整对话
